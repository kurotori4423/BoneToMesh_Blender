...
class KUROTORI_OT_active_bone_to_mesh(bpy.types.Operator):
    ...
    def execute(self, context):
        if context.mode == 'EDIT_ARMATURE':
            ...
            mat_X_Red.diffuse_color = (1.0, 0.0, 0.0, 1.0)
            
            mat_Z_Blue = bpy.data.materials.new('Z_Blue')
            #mat_Z_Blue.diffuse_intensity = 1.0
            mat_Z_Blue.diffuse_color = (0.0, 0.0, 1.0, 1.0)
...
classes = [
    KUROTORI_OT_active_bone_to_mesh
]
...
def register():
    bpy.utils.register_class(KUROTORI_OT_active_bone_to_mesh)
    bpy.types.VIEW3D_MT_edit_armature.append(menu_funk)

def unregister():
    bpy.utils.unregister_class(KUROTORI_OT_active_bone_to_mesh)
    bpy.types.VIEW3D_MT_edit_armature.remove(menu_funk)
